...
@section('content')
    ...
    <style>
        ...
        .edit-header h2 { margin: 0; font-size: 1.35rem; font-weight: 800; }
        .edit-header p { margin: 5px 0 0 0; opacity: 0.85; font-size: 0.85rem; }
    ...
    <div class="top-bar">
        <div>
            ...
            <p class="page-subtitle">Modifique el horario de su consulta médica.</p>
        </div>
    ...
    <div class="edit-container" x-data="rescheduleWizard()" x-init="init()">
        <div class="edit-header">
            <h2>Cambio de Bloque Horario</h2>
            <p>Seleccione un nuevo día y hora para ser atendido por su especialista.</p>
        </div>

        <div class="edit-body">
            
            <div class="current-details">
                <h3 style="margin: 0 0 10px 0; font-size: 1rem; font-weight: 700; color: var(--text-main);">Detalles de la Cita Actual</h3>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; font-size: 0.9rem;">
                    <div>
                        ...
                        Dr(a). {{ $cita->professionalProfile->user->name }}
                    </div>
                    <div>
                    ...
                    <div>
                        <strong style="color: var(--text-muted);">Horario Reservado:</strong><br>
                        {{ \Carbon\Carbon::parse($cita->start_datetime)->format('d/m/Y - H:i A') }}
                    </div>
                </div>
            </div>

            <form id="rescheduleForm" action="{{ route('citas.update', $cita->id) }}" method="POST">
                @csrf
                @method('PATCH')
                <input type="hidden" name="start_datetime" x-model="selectedDatetime">
            </form>

            <h4 style="margin: 0 0 15px 0; font-weight: 700; font-size: 1rem; color: var(--text-main);">1. Elija la nueva fecha:</h4>
            ...
            <h4 style="margin: 25px 0 10px 0; font-weight: 700; font-size: 1rem; color: var(--text-main);">2. Horarios disponibles:</h4>
            ...
            <div x-show="!loadingAvailability">
                <!-- Si no se encuentra el médico en la disponibilidad o no tiene bloques libres -->
                <div x-show="slots.length === 0" style="text-align: center; padding: 30px; color: var(--text-muted); background: #f8fafc; border-radius: 10px; border: 1px solid var(--border-color);">
                    No hay horas de atención disponibles para este especialista en la fecha seleccionada. Intente con otro día.
                </div>
            ...
            <div x-show="selectedDatetime" style="margin-top: 35px; text-align: right; border-top: 1px solid var(--border-color); padding-top: 20px;">
                <button 
                    ...
                    onclick="if(confirm('¿Confirmar la reprogramación de su cita?')) document.getElementById('rescheduleForm').submit()"
                >
    ...
    <script>
        function rescheduleWizard() {
            return {
                ...
                doctorId: {{ $cita->professional_profile_id }},
                dates: [],
                selectedDate: null,
                ...
                fetchAvailability() {
                    ...
                    .then(res => res.json())
                    .then(data => {
                        // Buscamos el registro del médico correspondiente
